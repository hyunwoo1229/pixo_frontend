...
export default function ReservationCard({ reservation, isOpen, onToggle }) {
  ...
  const renderDdayText = () => {
    ...
    if (dDay === 0) return <p className="rh-dday today">***오늘이 회의 날짜입니다***</p>;
    return <p className="rh-dday">***회의 날짜까지 {dDay}일 남았습니다***</p>;
  };
    ...
    <div className="rh-card">
      ...
      {isOpen && (
        <div className="rh-card-body">
          ...
          <dl>
            <dt>회의 시간:</dt>
            <dd>{formatTimeSlot(reservation.time)}</dd>
          </dl>
          <dl>
            <dt>희망 촬영 날짜:</dt>
            <dd>{reservation.desiredShootDate || "-"}</dd>
          </dl>
          <dl>
            <dt>희망 촬영 장소:</dt>
